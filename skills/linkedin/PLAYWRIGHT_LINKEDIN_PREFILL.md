...
脚本连接已运行的 FlashID 浏览器，在每个客户的新标签页中核对个人资料、打开“加为好友”并填入邀请备注。普通个人资料页优先使用资料卡上直接显示的 Connect/加为好友按钮或链接；Sales Navigator lead 客户信息页先尝试从二次菜单的 View LinkedIn profile 跳转到 `/in/...` 个人主页，再复用普通资料页流程；旧版菜单直接提供 Connect 时仍兼容。脚本没有发送逻辑，不会点击 LinkedIn 的“发送”按钮，也不会关闭 FlashID 浏览器或已有标签页。
