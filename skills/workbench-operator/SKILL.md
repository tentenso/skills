---
name: workbench-operator
description: 通过已配置的访问地址安全操作工厂客户开发工作台。用于查询客户和业务结果、操作页面、导入客户 CSV 或聊天记录、处理工作台 JSON，以及批量更新工厂业务数据。
---

# 工作台业务操作员

协助业务员或 Agent 使用工厂客户开发工作台。优先保护业务数据、工厂隔离、内容真实性、可恢复性和可核验结果。

## 首次初始化访问地址

每次启用 Skill 时，先解析工作台地址，不得根据浏览器历史、最近使用的 IP 或常见端口猜测：

```bash
<skill-dir>/scripts/workbench-config.sh resolve
```

- 如果命令成功，使用输出的无尾斜杠地址作为本次 `BASE_URL`。
- 如果尚未配置，先只询问用户“工作台访问地址”，然后执行：

```bash
<skill-dir>/scripts/workbench-config.sh init "http://approved-host:port"
```

- 初始化会验证 URL、探测首页并将地址写到用户级配置目录，文件权限为 `600`。
- `WORKBENCH_BASE_URL` 环境变量优先于配置文件，适合临时切换环境；不得把地址硬编码到 Skill 或业务文件。
- 开始操作前执行 `<skill-dir>/scripts/workbench-config.sh check`。不可达、证书错误或跳转异常时停止写操作并报告。
- 配置地址不代表批准修改部署边界。若项目文档仍限制为 `127.0.0.1`，不得自行改成局域网监听；部署调整属于独立的架构与安全任务。

## 先加载当前项目规则

在仓库内操作时读取：

1. [AGENTS.md](../../../AGENTS.md)：数据、部署和写入红线。
2. [DATA_FORMATS.md](../../../DATA_FORMATS.md)：CSV、聊天和工作台 JSON 格式。
3. [README.md](../../../README.md)：业务状态和日常流程。
4. 涉及启动、备份、数据库或部署时读取 [DEPLOYMENTS.md](../../../DEPLOYMENTS.md)。

文档与实现不一致时停止写入，查验当前代码，不选择更宽松的解释。

## 选择真实可用的接口

先阅读 [references/ui-and-http.md](references/ui-and-http.md)，按任务选择入口：

- 当前稳定的机器可调用 HTTP 接口只有 `GET /api/export`，返回全部工厂的敏感规范化导出。
- 页面写操作由 Next.js Server Actions 实现。Action ID 随构建变化，不是公共 API；不得抓取或硬编码 Action ID，也不得宣称存在未实现的 JSON 写接口。
- 远程 Agent 执行写操作时必须使用浏览器页面和可见控件。按标签、字段名和页面结果操作，不依赖 CSS 顺序或内部 Action ID。
- 若任务需要稳定的批量写 API，而当前页面没有对应批量入口，停止并报告产品能力缺口；不要用 SQLite、临时 SQL 或一次性脚本绕过。

批量导入或数据转换另读 [references/data-workflows.md](references/data-workflows.md)。只读取与当前任务相关的参考文件。

## 判断授权范围

把请求归为以下一种，并只执行对应范围：

- **查询/导出**：读取页面或规范化导出，不修改业务数据。
- **整理/转换**：生成预览或兼容数据，不写工作台。
- **预览导入**：在页面完成预览并报告，不点击最终确认。
- **实际导入或批量更新**：完成预览、备份和用户最终确认后写入。
- **单条业务记录**：通过对应页面新增或修改，并核对结果。
- **发送或结果判定**：发送外部消息、记录已发送、回复分类、停止联系、商机输赢必须获得该次具体确认。
- **代码/部署维护**：退出日常业务操作流程，按项目部署规则执行。

用户允许 Agent 使用系统，不等于允许任意批量写入、发送消息或改变业务判断。

## 每次操作的共同步骤

1. 运行 `git status --short`，保留用户已有修改。
2. 解析并检查 `BASE_URL`；记录实际访问环境。
3. 涉及业务数据时明确目标工厂名称和页面中的工厂 ID。目标不明确就停止询问。
4. 读取当前状态，确定记录 ID 和工厂归属；不得仅凭姓名选择客户。
5. 写操作前复述将变化的记录、字段和数量。批量操作必须先预览并备份。
6. 只使用页面提供的入口执行；不得直接编辑 SQLite。
7. 提交后重新读取对应页面或导出，核对实际结果、错误、跳过和工厂隔离。

## 真实性和并发边界

- 不虚构客户身份、联系方式、公开资料、互动、回复、需求、评分证据或商机金额。
- 不把草稿记为已发送，不把出站消息写成客户回复，不自动将 `PENDING` 改为 `QUALIFIED` 或 `MISMATCH`。
- 不向外部转换网站、公共 AI 或未经批准的服务上传客户数据。
- 无鉴权环境中的任何可访问者都可能读取或修改业务数据。执行批量操作前确认没有其他业务员或 Agent 正在处理同一批记录；发现页面状态与预览不一致时停止并重新预览。
- 当前 SQLite 工作台没有面向多 Agent 的任务锁、幂等键或冲突解决 API。不得把“请求返回成功”当作并发安全证明。

## 完成报告

```text
完成范围：查询 / 仅格式化 / 已预览 / 已导入 / 已更新
工作台地址：...
目标工厂：...
输入与结果：原始 ...，有效 ...，写入 ...，跳过 ...，错误 ...
备份：未涉及写入 / <实际备份路径>
核对：<重新读取的页面、导出统计和工厂隔离结果>
未处理项与原因：...
```

只报告实际执行并观察到的结果。
